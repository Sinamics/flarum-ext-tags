...
export default class TagDiscussionModal extends Modal {
  ...
  title() {
    return this.props.discussion
      ? app.translator.trans('sinamics-tags.forum.choose_tags.edit_title', {title: <em>{this.props.discussion.title()}</em>})
  ...
  getInstruction(primaryCount, secondaryCount) {
    if (primaryCount < this.minPrimary) {
      ...
      return app.translator.transChoice('sinamics-tags.forum.choose_tags.choose_secondary_placeholder', remaining, {count: remaining});
    }
